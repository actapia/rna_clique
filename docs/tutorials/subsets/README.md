...
For reference, the genotypes of the samples analyzed in the "From RNA-seq reads
to a phylogenetic tree with RNA-clique" tutorial are shown below:
...
Suppose we wanted to look only at the CTE46 and CTE27 samples. Of course, we
could just take the rows and columns corresponding to the CTE samples from of
the distance matrix from our full analysis. In general, this approach is not
the best option because we can often obtain more ideal components with smaller
subsets of samples, and simply taking the distances directly from the existing
distance matrix forces us to use the ideal components from the full (supserset)
...
Actually, for this particular case, obtaining the CTE-only distance matrix
directly from the full distance matrix might be a reasonable option. We have
plenty of ideal components, and it doesn't seem that the other genotypes are so
distant from the CTE genotypes that the distances among the CTE genotype samples
are obscured. For the sake of example, however, we'll assume that we need to
recompute the distances for the set of CTE samples. Although the recomputation
may not be necessary, it would be interesting to see how many more ideal
components we obtain when analyzing only the CTE samples.


A naive way of recomputing a distance matrix for just the CTE samples would be
to re-run the command in the ["Running
...
this approach is straightforward, it be inefficient because we already did
most of the work needed to recompute this distance matrix when we ran the
original subset analysis. If we were to re-run RNA-clique with just the CTE
samples, RNA-clique would need to first select the top $n$ genes from the CTE
samples, then obtain gene matches tables for each pair of CTE samples, and these
steps would account for the majority of the running time of RNA-clique. 

In the superset analysis, however, RNA-clique also selected the top $n$ genes
from the CTE samples (along with all of the other samples) and computed the gene
matches tables for every pair of CTE samples (and every other pair of
...
the pairs of CTE samples, `make_subset.py` can build a new gene matches graph,
...
samples to include. For this example, we'll specify the CTE samples directly on
the command line.
...
python make_subset.py -I "$TUTORIAL_DIR"/rna_clique_out/config.yaml \
                      -O "$TUTORIAL_DIR"/subset_out \
                      -y SRR2321388 SRR2321385 SRR8003761 SRR8003762
```
...
`"$TUTORIAL_DIR"/subset_out`, you'll see that we don't have a distance matrix
yet. To get the distance matrix, we also need to run `filtered_distance.py` on
the subset analysis.

```bash
python filtered_distance.py -O "$TUTORIAL_DIR"/subset_out
```

If you check `"$TUTORIAL_DIR"/subset_out` again, you should now see a
`distance_matrix.h5` file.

We can also check the number of ideal components.

```bash
python plot_component_sizes.py --statistics -A "$TUTORIAL_DIR"/subset_out
```

You should see that we have many more ideal components (around $14164$) with
just the four CTE samples.
